...
- **QA instrument**: **SET UP 2026-08-21** — `/map-qa-instrument create` wrote the map (8 files), `/build-qa-bench` wrote the missing `qa/README.md` R/I/A/O index and promoted all four phases from `tribal` to `documented` by linking the five scripts that already existed. The loop was run end to end before any status flipped: RESET clean, INJECT 27 agents / 56 shared / 1,401 memory, ACT booted, OBSERVE `SMOKE OK` exit 0. *(The earlier auto-skip recorded here was correct at the time — [USER-NAME] noticed the missing checklist and set the instrument up rather than accepting the skip.)*
- **Checklist**: [qa/checklists/user-profile.md](../qa/checklists/user-profile.md) — risk-derived rather than plan-restating, with a state model up front (no record / blank field / present) because most items turn on telling those apart.
- **Coverage split**: **19 automated** (named per row in the checklist's coverage table) / **~20 manual**, of which the load-bearing group is the **first-run bootstrap** — unreachable by any test. The coverage table is scoped to what each test can *fail on*, not what it asserts: the shell configurator, the compile pipeline, `search` labelling, and the two-profiles-before-the-index case are all marked still-manual.
- **Runtime verification**: **NOT DONE.** Next action: `/run-qa-test --checklist qa/checklists/user-profile.md` with the stack up. The bootstrap items additionally need a **fresh agent session** reading the installed instruction — commit, run the core **and** overlay installers as a pair, then awaken.
