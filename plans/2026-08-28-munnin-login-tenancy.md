...
- [x] A real token from the live issuer is accepted by the deployed server, and its audience is confirmed — **by acceptance rather than by decoding**, which is the stronger form: Munnin returns 200 only after checking signature, issuer *and* `aud`, so an accepted call proves what a decoded claim would merely describe.
- [x] `alvi` remains the internal identifier, and the reimported memory is reachable only by the identity mapped to it — 27 agents / 1,441 records, reached from a live agent session and refused to everyone else.
...
- [x] **Step 4.2**: Configure the issuer
...
- [x] **Step 4.3**: Deploy onto a recreated volume, and decode the token
...
- [x] **Step 4.5**: Re-authenticate the local agent sessions
...
- [x] **Step 4.2**: Configure the issuer
  - **Implementation Log**: Alvi's, in the WorkOS dashboard. Google enabled as a social connection using WorkOS's demo credentials (his own Google Cloud credentials deferred — they change the consent screen's branding and nothing else); Client ID Metadata Document enabled; Dynamic Client Registration enabled alongside it for older clients; the server URL registered as a Resource Indicator; sign-up left invitation-only. 🚨 **Two of the six instructions I wrote him were wrong, and both trace to one source**: I took them from `AuthKitProvider`'s docstring in the installed FastMCP, which documents setup for `WorkOSProvider` — the OAuth **proxy** class — not the metadata-forwarding provider this server actually uses. I sent him to *"Applications → Configuration → Dynamic Client Registration"* when the current docs say **CIMD, under Connect → Configuration**, and told him to add a redirect URI that this provider does not need at all. He discovered it by failing to find them. A vendor SDK's docstring is a claim about somebody else's UI, and it ages independently of the code around it — reading the vendor's own docs is the check I skipped.
  - **Testing Log**: Both toggles were then **verified from outside the dashboard**, without needing his screen: the AuthKit metadata Munnin forwards reads `"client_id_metadata_document_supported": true` and carries a `registration_endpoint`, so CIMD and DCR are both live, and its `issuer` matches exactly what Munnin verifies against. The Resource Indicator is **not** observable there — no OAuth server metadata carries one — and was settled instead by Step 4.3's accepted token.
  - **Success Criteria**: **Pass.** A real token was obtained outside claude.ai, through MCP Inspector against the deployed server.
  - **Tech Debts**: This is the **staging** environment. Every setting above must be repeated in Production, whose AuthKit domain will differ — and the identity row written in Step 4.4 carries the staging `iss`, so that cutover is a data change as well as a config change.
  - **Result**: Met.
- [x] **Step 4.3**: Deploy onto a recreated volume, and decode the token
  - **Implementation Log**: `agent-memory-server@2a3efba` and `munnin-deploy@85b7298` pushed, each confirmed on the remote with `git ls-remote` rather than a tracking ref. `config/deploy.yml` gained `MUNNIN_AUTHKIT_DOMAIN` and `MUNNIN_PUBLIC_BASE_URL` in `env.clear` — no secret is involved, since a resource server holds only a public JWKS — plus a comment recording that `MUNNIN_DOCS` is deliberately absent. The image tag was verified to **exist in GHCR** by an anonymous manifest request returning 200, rather than inferred from a green build; `588c59a` is still present as a rollback target. Old container and volume removed, deploy run 33240887745 succeeded, `munnin-web-2a3efba` healthy on a fresh volume.
  - **Testing Log**: 🚨 **The volume recreation was proven necessary, not assumed.** Before deleting, the live database was read through the running container: **no `account` table at all**, and `PRAGMA foreign_key_list(agent)` returning `[]`. Since the schema is applied with `CREATE TABLE IF NOT EXISTS`, reuse would have kept that table unconstrained forever with nothing reporting it. Deleting was equally proven safe — `agent`, `memory_record` and `shared_record` all held **0 rows**. After the deploy, the chain is live on the real store: `agent → account(user_id)`, `user_identity → account(user_id)`, and the composite `memory_record → agent(user_id, agent_id)`. **A behaviour worth recording: the schema is applied lazily, on the repository's first connection** — a freshly booted container has *no tables at all*, so a naive constraint check immediately after deploy reads exactly like a missing constraint. **The before/after probe, anonymous, against the live host**: `/api/agents` 200 → **401**, `/api/prompts` 200 → **401**, `/api/awaken` → **401**, `POST /mcp/` 200 → **401**, `/openapi.json` 200 → **404**, `/docs` → **404**, discovery 404 → **200**, `/health` 200 → 200. The `WWW-Authenticate` header now names a URL that resolves, and the document it returns reads `{"resource": "https://munnin.lok.quest/", "authorization_servers": ["https://secure-sweetness-13-staging.authkit.app/"]}` — the resource string matching exactly what must be registered as the Resource Indicator.
  - **Success Criteria**: **Pass, and by a stronger route than the plan specified.** The previously-open probe returns 401. The audience was then settled by Alvi completing the flow in MCP Inspector, and the server's own log is the evidence, read in order: `POST /mcp/` **401** → `GET /.well-known/oauth-protected-resource` **200** → `POST /mcp/` **200**. That final 200 is only reachable after Munnin has verified signature, issuer **and** `aud` against `https://munnin.lok.quest/`, so an accepted call proves the Resource Indicator is registered correctly — which a decoded token would only have *described*. The plan asked for a decode specifically to avoid mistaking a green login for a working audience; an accepted API call clears that bar outright. Note the middle line: that discovery request is the one that returned 404 before this session's fix, so the login could not have completed at all without it.
  - **Tech Debts**: The deployed store now has a schema but no tenant and no memory, so `awaken` answers empty for everyone until the import runs. The `probe` nginx container is still running on the box (carried, untouched). The AuthKit tenant is a **staging** environment — the production cutover is now a tracked item rather than a memory, since its domain will differ and every dashboard setting must be repeated there.
  - **Result**: Deployed and closed. 🚨 **The live server is no longer world-writable** — the standing exposure carried since 2026-08-28 is ended.
- [x] **Step 4.4**: Import the memory
  - **Implementation Log**: Imported the fleet locally into a scratch database with `MUNNIN_USER_ID=alvi`, linked Alvi's identity to that tenant through the real write path, then shipped the file. **His `(iss, sub)` never had to be pasted**: the first authenticated store-touching call wrote `new tenant created: user_id=… iss=… sub=…` at WARNING, exactly as designed, so the server told us who he was — `sub=user_01M167EJGMYJ50H2M8R4M4WWBE` under the staging issuer. That also retired the auto-created tenant without a separate cleanup step, since replacing the whole file leaves only `alvi`. Transfer verified by **sha256 on both ends** (`cf1d69ae…`), not by file size. The swap ran through a helper container against the named volume with the container stopped — WAL sidecars removed rather than left to confuse the next open — and ownership was **read from the existing file (10001:10001) rather than assumed**. Staging copy deleted from the host afterwards, so the fleet's memory is not sitting in a home directory.
  - **Testing Log**: Live store after the swap: account `alvi` alone, the identity row mapping the staging issuer and Alvi's subject to it, **27 agents / 1,441 memory records / 60 shared records** — up from the 27 / 1,399 / 55 recorded on 2026-08-20, which is the fleet having grown rather than a discrepancy. Before shipping, `resolve(iss, sub)` was proved locally to return **`alvi`** *and* to create **no second account**, so it found the mapping rather than minting alongside it. The guard was re-checked after the restart and holds: `/api/agents` 401, `/api/awaken` 401, `POST /mcp/` 401, `/openapi.json` 404, `/health` 200. 🚨 **A path trap worth recording, and a fourth recurrence of the line-endings/paths class**: MSYS translates `/tmp/x` when it is a bare argument to a native Windows program but **not** inside a quoted Python string, so the importer wrote 19 MB to `%TEMP%` while the verification read an empty file at `C:	mp` and reported *"no such table: account"* — a missing-schema symptom with a path cause. Both now use explicit Windows paths.
  - **Success Criteria**: **Pass, both halves.** The memory is present and reachable only through the mapped identity, and Alvi confirmed from Inspector that `list_agents` now returns **27 agents** where it had returned an empty list an hour earlier — the same call, the same token, the difference being only that the store now holds his fleet and his identity maps to it.
  - **Tech Debts**: The import ran from a laptop and the database was shipped as a file; there is no repeatable path for doing this on the box, which matters the next time the store is rebuilt. The **staging** issuer is now baked into a live identity row — when the production AuthKit tenant is cut over, that row's `iss` changes and the mapping must be re-pointed, or Alvi's own memory becomes unreachable to him. That is the sharpest consequence of the staging-first choice and it is now written down rather than remembered.
  - **Result**: Met server-side. The memory that had been deliberately blocked since 2026-08-28 is live behind a login.
- [x] **Step 4.5**: Re-authenticate the local agent sessions
  - **Implementation Log**: Nothing to build — Alvi ran `/mcp` in an interactive session, which performs the OAuth flow once and caches the token in the system keychain. It could not be done from an agent session, which is non-interactive by construction and cannot complete a browser redirect.
  - **Testing Log**: The break was **proven before the fix, not predicted**: the `munnin` tool returned *"requires re-authorization (token expired)"* — a legible failure that names its own remedy. After the flow, `ping` returned `pong`, which proves only the transport, and then `list_agents` returned **27 agents including `software-architect`**, which is the number the store holds and the proof that matters: `ping` touches no store and so resolves no tenant, while `list_agents` runs the whole chain — token verified for signature, issuer and audience, `(iss, sub)` resolved through `user_identity` to `alvi`, and the query scoped to that tenant.
  - **Success Criteria**: **Pass.** A local agent session reaches its own fleet memory through the deployed server with **no machine credential anywhere** — decision 10 demonstrated rather than asserted.
  - **Tech Debts**: None.
  - **Result**: Met. The plan is complete.
