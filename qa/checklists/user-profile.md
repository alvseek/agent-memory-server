...
- [x] Run it normally, keep all three values: the private file and `output/0-core-user-profile.md` are byte-identical afterwards. *(2026-08-21 — the configurator was **run for real** as part of the installer pair; it rewrote both files at 15:53:47 and they are byte-identical, and were also identical beforehand.)*
- [x] Compile and compare the output's profile block against live `CLAUDE.md` — **byte-identical**. The source moved; the permanent layer must not. *(2026-08-21 — **run for real**. The compile regenerated `core-memory-compiled.md` at 15:53:48; live `CLAUDE.md` is that output **plus exactly 148 bytes**, which is the overlay's path-definition line and its blank separator. Live `CLAUDE.md` came out **byte-identical to the pre-run backup** — same md5 — so the source move cost the permanent layer nothing. Both installers were run as a pair; 54 commands installed, 16 core + 38 overlay.)*
...
**Sign-off**: **NOT SIGNED OFF** — the payload-cap row failed, and five rows were never reached: the four first-run-bootstrap behaviours need a fresh agent session, and the MCP-face comparison was not walked live (its automated twin is green). The two pipeline rows were **later run for real** (2026-08-21 15:53) after the branch was merged to `main` and the store checkout brought forward; both passed.
...
| 7 | ~~The two `control-files` checkouts have diverged~~ — **retracted, the check was faulty.** The ancestry test ran inside the store's checkout, which had never fetched `user-profile-record`; `git merge-base --is-ancestor` exits non-zero for an *unknown object* exactly as it does for *not an ancestor*, and the test read the first as the second. Re-run where both objects exist, `origin/main` is an ancestor of the branch tip in **both** repos — the merge is a fast-forward. The store checkout being behind is real and is just an unfetched checkout, not divergence. | qa | **retracted** 2026-08-21 |
