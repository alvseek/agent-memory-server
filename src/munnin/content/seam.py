...
submodule (``procedures/memory/storage-backends/compose.py``) — Munnin-agnostic, so
...
_COMPOSE_REL = "procedures/memory/storage-backends/compose.py"


@lru_cache(maxsize=None)
def seam_compose(content_root: str) -> ModuleType:
    """Load + cache the canonical ``compose`` module from a control-files root.
    ...
    """
    path = Path(content_root) / _COMPOSE_REL
    if not path.exists():
